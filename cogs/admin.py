...
class Admin(commands.Cog):
    ...
    @servers.error
    async def servers_error(self, interaction: discord.Interaction, error):
    ...
    async def create_invite(self, interaction: discord.Interaction, server_id: int):
        ...
        if guild is None:
            await interaction.response.send_message(get_text(guild_id, 'server_not_found'))
            return
        
        text_channels = [ch for ch in guild.text_channels if ch.permissions_for(guild.me).create_instant_invite]
        
        if not text_channels:
            await interaction.response.send_message(get_text(guild_id, 'no_permission_invite', guild.name))
            return
        ...
        try:
            ...
            await interaction.response.send_message(f"❌ Error creating invite: {e}")
    
    @create_invite.error
    async def create_invite_error(self, interaction: discord.Interaction, error):
        if isinstance(error, commands.CheckFailure):
            guild_id = interaction.guild.id if interaction.guild else None
            await interaction.response.send_message(get_text(guild_id, 'owner_only'))
