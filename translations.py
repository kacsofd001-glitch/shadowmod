...
TRANSLATIONS = {
    ...
    'hu': {
        ...
        'help_moderation_desc': '`/ban` `/kick` `/mute` `/unmute` - Felhasználó kezelés\n`/tempmute <user> <idő>` - Discord timeout (max 28 nap)\n`/purge <1-100>` - Tömeges üzenet törlés\n`/warn` `/lock` `/unlock` - Figyelmeztetések & csatorna vezérlés',
        'help_games': '🎮 Szórakoztatás',
        'help_games_desc': '`/rps` - Kő Papír Olló • `/tictactoe` - Amőba\n`/meme` - Véletlen meme • `/8ball` - Varázs labda\n`/coinflip` - Pénzfeldobás • `/dice` - Kockadobás',
        'help_engagement': '🎁 Közösségi',
        'help_engagement_desc': '`/poll` - Interaktív szavazások • `/giveaway` - Nyereményjátékok\n`/ticket` - Support jegyek • `/createrole` - Szerep kezelés',
